...
public class RootActivity extends ListActivity {
	...
	public void onrPermBtnPress(View v) {
		...
		if(NumberofSelectedItems > 1) {
			showToast(getString(R.string.usepfm));
			findViewById(R.id.rCopyBtn).setVisibility(View.INVISIBLE);
			findViewById(R.id.rPasteBtn).setVisibility(View.INVISIBLE);
			findViewById(R.id.rMoveBtn).setVisibility(View.INVISIBLE);
			findViewById(R.id.rDeleteBtn).setVisibility(View.INVISIBLE);
		...
		aDialog.setPositiveButton(getString(R.string.Finish),
				new DialogInterface.OnClickListener() {
					public void onClick(DialogInterface dialog, int which) {
						...
						findViewById(R.id.rCopyBtn).setVisibility(
								View.INVISIBLE);
						findViewById(R.id.rPasteBtn).setVisibility(
								View.INVISIBLE);
						findViewById(R.id.rMoveBtn).setVisibility(
								View.INVISIBLE);
						findViewById(R.id.rDeleteBtn).setVisibility(
								View.INVISIBLE);
						findViewById(R.id.rPermBtn).setVisibility(
								View.INVISIBLE);
					}
				});
		...
		if (from.isFile())
			...
			String dir = to.getAbsolutePath()
					+ from.getAbsolutePath().substring(
							...
							from.getAbsolutePath().length());
			int len = files.length;

			new RootFile(dir).mkdir();

			for (int i = 0; i < len; i++) {
				if ((new RootFile(from.getAbsolutePath() + "/"
						+ files[i].getName())).isDirectory())
					FileCopy(new RootFile(from.getAbsolutePath() + "/"
							+ files[i].getName()), new RootFile(dir));

				else
					RootTools.copyFile(
							from.getAbsolutePath() + "/" + files[i].getName(),
							dir, false, true);
			}
		}
	}
	...
	public class FileAdapter extends BaseAdapter {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if (nowPath.equals(root))
				sb.append(nowPath).append(object.get(position).getName());
			else
				sb.append(nowPath).append("/")
						.append(object.get(position).getName());
