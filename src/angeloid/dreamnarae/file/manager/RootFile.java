    ...
    public RootFile[] listFiles() {
        ...
        try 
        {
            ...
            for(String line : outLines) 
            {
                ...
                line = getName(line);
                if(!line.equals(".") && !line.equals("..")) dirContents.add(line);
            }
        } catch(Exception e) { Log.e("Exception", e.getMessage()); }
        
        String[] sf = dirContents.toArray(new String[0]);
        ArrayList<RootFile> files = new ArrayList<RootFile>();
        for(int i = 0; i <= sf.length-1; i++)
	    {
	    	files.add(new RootFile(actualPath + sf[i]));
	    }
